...
contract NineGods {
    ...
    uint256 private rndInc_ = 1800 seconds;              // 每次买key增加3600s 60min
    ...
    tokenODF constant ODF = tokenODF(0xAB4E1802c61e12fd7b10a69A226F5D727c76a8Aa);
    ...
    constructor() public {
        ...
        nRollIn = 73747;
        nCurrentGainId = 53635;
        teamPot.total = 14749400000000000000000000;
        teamPot.potCoin = 1474940000000000000000000;
    }
    ...
    function createPlayer(address _addr, address _referrer) private {
        ...
        if (pIDxAddr_[_addr] == 0) {
            ...
            playerEtraxAddr_[nPlayerNum] = DataSets.PlayerEtra(0, 0, round, 0, 0, 0, 0, 0, 0, 0, 0);
        }
    }
    ...
    */
    function luckyOver() private {
        ...
        //要分的金额
        uint totalPot = rate[5] * nBugBonus * 5 * rate[4] / 10000 + teamPot.potCoin * rate[11] / 1000;
    ...
    function destory() public {
        if (msg.sender == creator) {
            transfer(banker[0], ODF.balanceOf(this));
        }
    }
    ...
    function updateRollIn(uint _begin, uint [130] id) public {
        require(canFix);
        uint a = _begin;
    ...
    function fixedData(address [25] _addr, uint [325] _other) public {
        require(canFix);
        for (uint i = 0; i < 25; i++) {
            ...
            playerxID_[id].turnBonus = _other[13 * i + 1];
            playerxID_[id].currentBonus = _other[13 * i + 2];
            playerxID_[id].reinvest = _other[13 * i + 3];
            playerxID_[id].currentInterest = _other[13 * i + 4];
            playerxID_[id].agents = _other[13 * i + 5];
