...
const json = require('./NineGods.json');
let ContractAddr = "0x13552c7cc9ce39af665955412faa08f0e6555a29";
...
function connectionMysql() {
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            setInterval(getLuckyer, 30 * 60 * 1000);
        }
    });
...
function getLuckyAddress(index, coin) {
    ContractInstance.methods.luckyer(index).call().then((id) => {
        console.log(id, coin);
        let sql = 'insert into luckyId (userId,bonus,time) values(?,?,?) ;';
