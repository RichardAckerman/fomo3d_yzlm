...
contract XMan {
    ...
    function coinRollIn(uint _team, address _referrer) isWithinLimits(msg.value, msg.sender) public payable {
        ...
        //data change
        if (playerxID_[pIDxAddr_[_addr]].allBuy > 0 && playerxID_[pIDxAddr_[_addr]].reinvest < 10) {// 不是第一次买
    ...
    function bonusToReferrer(uint allCoin, address _addr) private {
        ...
        uint myReinvest = playerxID_[myId].reinvest;
        // 我的复投次数
        ...
        if (referees != emptyAddr) {
            ...
            if (myReinvest == 1) {
                transferCoin(referees, allCoin * 5 / 100);
            }
            ...
            for (uint j = 0; j < 9; j++) {
                ...
                if (agentId > 0) {
                    ...
                    if (j == 5 && level >= 5) {
                        ...
                        if (myReinvest == 5) {
                            transferCoin(playerxID_[agentId].addr, allCoin * 5 / 100);
                        }
                    }
                    if (j == 6 && level >= 4) {
