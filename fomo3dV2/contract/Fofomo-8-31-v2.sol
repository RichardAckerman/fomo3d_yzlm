...
contract Fofomo is modularLong {
    ...
    constructor()public{
        ...
        totalWithDrawNum = 121171355532426601733;
        totalKey = 2651313;
        totalKeyBonus = 410177024796000000000;
        overMoment = 1535832651;
        isBegin = true;
        teamPot[0].total = 20978395700000000000;
        teamPot[0].potCoin = 1468487699000000000;
        teamPot[1].total = 120184173600000000000;
        teamPot[1].potCoin = 8412892152000000000;
        teamPot[2].total = 1714363776100000000000;
        teamPot[2].potCoin = 120005464327000000000;
        teamPot[3].total = 97701125600000000000;
        teamPot[3].potCoin = 6839078792000000000;
        teamPot[4].total = 1953227471000000000000;
        teamPot[4].potCoin = 760419482187721251;

        totalTotalInvest = getTotalInvest();
        keyPriceCurrent = totalTotalInvest / (1 ether) * keyPriceInitial_ / 100 + keyPriceInitial_;
        initData = totalTotalInvest / (1 ether) * keyPriceInitial_ / 100 + keyPriceInitial_;
    ...
    function setOtherData(uint id, address _addr, bytes32 userName, uint refereesBonus, uint performance, uint level, uint currentRound) private {
        playerEtraxID_[_addr] = FofDataSets.PlayerEtra(userName, refereesBonus, performance, level, currentRound, 0);
    ...
    function updateCWithdraw(address _addr) private {
        uint cWithdraw = 0;
        if ((playerxID_[pIDxAddr_[_addr]].keyNum * keyDividend >= playerxID_[pIDxAddr_[_addr]].costNum + playerxID_[pIDxAddr_[_addr]].withDrawNum)) {
            cWithdraw = playerxID_[pIDxAddr_[_addr]].costNum;
        } else {
            if (playerxID_[pIDxAddr_[_addr]].keyNum * keyDividend > playerxID_[pIDxAddr_[_addr]].withDrawNum)
            {
                cWithdraw = playerxID_[pIDxAddr_[_addr]].keyNum * keyDividend - playerxID_[pIDxAddr_[_addr]].withDrawNum;
            } else
            {
                cWithdraw = 0;
            }
        }
        playerEtraxID_[_addr].cWithdraw = cWithdraw;
    }
