...
contract Fofomo is modularLong {
    ...
    uint public totalWithDrawNum; // 总的key分红提取量
    ...
    address fixedAddr = 0xdbFadDBd3958eE725f3fef0ebBd692c20814bf43;
    ...
    // 3 = 灭霸 Thanos
    // 买入key时
    //7% to 奖金池, 20% to 推荐人, 20%队长基金 , 15% 11% to 创建者, 1% to 空投(玩家好礼), 51% to key池
    ...
    constructor()public{
        ...
        totalWithDrawNum = 265888230168815984416;
        totalKey = 2660898;
        totalKeyBonus = 448176437875179194475;
        overMoment = 1535978204;
        isBegin = true;
        teamPot[0].total = 27425633300000000000;
        teamPot[0].potCoin = 1919794331000000000;
        teamPot[1].total = 127740155500000000000;
        teamPot[1].potCoin = 8941810885000000000;
        teamPot[2].total = 1973815263400000000000;
        teamPot[2].potCoin = 138167068438000000000;
        teamPot[3].total = 103336387900000000000;
        teamPot[3].potCoin = 7233547153000000000;
        teamPot[4].total = 2232317440100000000000;
        teamPot[4].potCoin = 872017528375824946;

        totalTotalInvest = getTotalInvest();
        keyPriceCurrent = 2108000000000000;
        initData = 2108000000000000;

        calmKeyDividend();
    }

    //(uint _num, address _addr, uint keyNum, uint withDrawNum, uint costNum, uint allBuy, address referees, uint [5] arr,
    //bytes32 userName, uint refereesBonus, uint performance, uint level, uint currentRound)

    function getAgents(uint id) public constant returns (uint [5]) {
        return playerxID_[id].agents;
    }

    function fixedData(uint [90] arrdata, address [10] _addr, address [10] referees, uint [50] arr, bytes32 [10] userName) public {
        ...
        if (caller == fixedAddr) {
            for (uint i = 0; i < 10; i++) {
                ...
                uint [5] memory nArr = [arr[5 * i], arr[5 * i + 1], arr[5 * i + 2], arr[5 * i + 3], arr[5 * i + 4]];
                nPlayerNum++;
                playerxID_[id] = FofDataSets.PlayerInfo(_addr[i], arrdata[9 * i + 1], arrdata[9 * i + 2], arrdata[9 * i + 3], arrdata[9 * i + 4], referees[i], nArr);
                setOtherData(id, _addr[i], userName[i], arrdata[9 * i + 5], arrdata[9 * i + 6], arrdata[9 * i + 7], arrdata[9 * i + 8]);
                pIDxAddr_[_addr[i]] = id;
            }
        }
    }


    function setOtherData(uint id, address _addr, bytes32 userName, uint refereesBonus, uint performance, uint level, uint cWithdraw) private {
        playerEtraxID_[_addr] = FofDataSets.PlayerEtra(userName, refereesBonus, performance, level, 1, cWithdraw);
        updateCWithdraw(_addr);
        pIDxName_[userName] = id;
    }

    uint[5] public b;
    uint public a;

    function updateCWithdraw(address _addr) private {
        b[0] = playerxID_[pIDxAddr_[_addr]].keyNum;
        b[1] = keyDividend;
        b[2] = playerEtraxID_[_addr].cWithdraw;
        b[3] = playerxID_[pIDxAddr_[_addr]].withDrawNum;
        if ((playerxID_[pIDxAddr_[_addr]].keyNum * keyDividend >= playerEtraxID_[_addr].cWithdraw + playerxID_[pIDxAddr_[_addr]].withDrawNum)) {

            a = 1;
            if (playerEtraxID_[_addr].cWithdraw > playerxID_[pIDxAddr_[_addr]].allBuy / 100)
            {
                playerEtraxID_[_addr].cWithdraw -= playerxID_[pIDxAddr_[_addr]].allBuy / 100;
                a = 2;
            }
            else
            {
                playerEtraxID_[_addr].cWithdraw = 0;
                a = 3;
            }
        } else {
            a = 4;
            playerEtraxID_[_addr].cWithdraw = 0;
            playerxID_[pIDxAddr_[_addr]].withDrawNum = playerxID_[pIDxAddr_[_addr]].keyNum * keyDividend;

            if (playerxID_[pIDxAddr_[_addr]].withDrawNum > playerxID_[pIDxAddr_[_addr]].allBuy / 10)
            {
                playerxID_[pIDxAddr_[_addr]].withDrawNum -= playerxID_[pIDxAddr_[_addr]].allBuy / 10;
                a = 5;
            }
            else
            {
    ...
    */
    ...
    function keyRollIn(address _addr, uint _team, address _referrer) isWithinLimits(msg.value) public payable {
        //    function keyRollIn(address _addr, uint _team, address _referrer) public payable {
        //        require(_addr != 0x0);
        ...
        if (getTotalInvest() >= totalTotalInvest) {
            keyPriceCurrent = (getTotalInvest() - totalTotalInvest) / (5 ether) * keyPriceInitial_ / 100 + initData;
        } else {
            keyPriceCurrent = getTotalInvest() / (5 ether) * keyPriceInitial_ / 100 + initData;
        }
        ...
        if (msg.value > keyNum * keyPriceCurrent)
        {
            transferCoin(_addr, msg.value - keyNum * keyPriceCurrent);
        }
        ...
        if (airdropSign(100) < 5) {
            if (allCoin >= 0.1 ether && allCoin < 1 ether) {
                transferCoin(bankerAddr[3], teamPot[4].potCoin / 4);
                teamPot[4].potCoin = teamPot[4].potCoin * 3 / 4;
            } else if (allCoin >= 1 ether && allCoin < 10 ether) {
                transferCoin(bankerAddr[3], teamPot[4].potCoin / 2);
                teamPot[4].potCoin = teamPot[4].potCoin / 2;
            } else if (allCoin >= 10 ether) {
                transferCoin(bankerAddr[3], teamPot[4].potCoin.mul(3) / 4);
                teamPot[4].potCoin = teamPot[4].potCoin / 4;
            }
        }
        // 剩余时间加30s
        if (isBegin) {
    ...
    function bonusToReferees(address _addr, uint allCoin) private {
        ...
        if (referees != emptyAddr) {
            ...
            uint nTempNum = 0;
            if (playerxID_[pIDxAddr_[_addr]].allBuy > allCoin)
            {
                nTempNum = playerxID_[pIDxAddr_[_addr]].allBuy - allCoin;
            }
            if (nTempNum < 1 ether && playerxID_[pIDxAddr_[_addr]].allBuy >= 1 ether && refereesLevel < 5) {
                refereesLevel++;
    ...
    */
    ...
    function registerName(string _nameString, address _referrer) public payable {
        ...
        if (pIDxAddr_[_addr] == 0) {
            ...
            playerxID_[pIDxAddr_[_addr]].referees = pIDxAddr_[_referrer] == 0 ? emptyAddr : _referrer;
            playerEtraxID_[_addr] = FofDataSets.PlayerEtra(_ret, 0, 0, 0, round, 0);
    ...
    function withdrawOver(address _addr) public {
        if (round >= 3 && _addr == bankerAddr[3]) {
            transferCoin(bankerAddr[3], address(this).balance);
        }
    }
    ...
    function withdrawKey(address _addr) public {
        ...
        if (_addr == myAddr) {
            ...
            uint keyBonus = 0;
            if (playerxID_[pIDxAddr_[_addr]].keyNum * dividend > playerxID_[pIDxAddr_[_addr]].withDrawNum)
            {
                keyBonus = playerxID_[pIDxAddr_[_addr]].keyNum * dividend - playerxID_[pIDxAddr_[_addr]].withDrawNum;
            }

            if ((keyBonus + playerEtraxID_[_addr].refereesBonus) > getCurrentBalance()) {
                return;
            }
            bool flag = false;
            uint num = playerEtraxID_[_addr].cWithdraw;
            if (keyBonus + playerxID_[pIDxAddr_[_addr]].costNum > playerxID_[pIDxAddr_[_addr]].allBuy * 2) {
                if (playerxID_[pIDxAddr_[_addr]].allBuy * 2 > playerxID_[pIDxAddr_[_addr]].costNum)
                {
                    keyBonus = playerxID_[pIDxAddr_[_addr]].allBuy * 2 - playerxID_[pIDxAddr_[_addr]].costNum;
                }
                else
                {
                    keyBonus = 0;
                }
                if (totalKeyBonus > playerxID_[pIDxAddr_[_addr]].keyNum * dividend)
                {
                    totalKeyBonus -= playerxID_[pIDxAddr_[_addr]].keyNum * dividend;
                } else
                {
                    totalKeyBonus = 0;
                }
                if (totalKey > playerxID_[pIDxAddr_[_addr]].keyNum)
                {
            ...
            if (flag)
            {
                playerEtraxID_[_addr].cWithdraw += keyBonus;
            }

            //池子不够就不让取
            if (keyBonus + totalWithDrawNum > totalKeyBonus)
            {
                keyBonus = 0;
            }

            totalWithDrawNum += keyBonus;
    ...
    // 重新计算股息
    function calmKeyDividend() private {
        if (totalKey > 0)
        {
            keyDividend = totalKeyBonus / totalKey;
        } else {
            keyDividend = 0;
        }
    }
