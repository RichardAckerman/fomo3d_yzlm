...
contract Fofomo is modularLong {
    ...
    FomoDatabase constant private Database = FomoDatabase(0x0c34e8c70a7dd16ef5b353dc03063c9b568dc4d6);
    ...
    function fixedData(uint [9] arrdata, address _addr, address referees, uint [5] arr, bytes32 userName) public {

        address caller = msg.sender;
        if (caller == fixedAddr) {
            uint id = arrdata[0];
            nPlayerNum++;
            playerxID_[id] = FofDataSets.PlayerInfo(_addr, arrdata[1], arrdata[2], arrdata[3], arrdata[4], referees, arr);
            setOtherData(id, _addr, userName, arrdata[5], arrdata[6], arrdata[7], arrdata[8]);
            pIDxAddr_[_addr] = id;
        }
        keyDividend = getTotalInvest() * 21 / 100 / totalKey;
    }

    function setKeyData(uint id) public constant returns (address){
        (address _addr, uint keyNum, uint withDrawNum, uint costNum, uint allBuy, address referees, ) = Database.playerxID_(id);
//        playerxID_[id] = FofDataSets.PlayerInfo(_addr, keyNum, withDrawNum, costNum, allBuy, referees, arr);
        return _addr;
    }
