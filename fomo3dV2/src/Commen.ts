...
function getKeyDividend() {
    return new Promise((resolve, reject) => {
        $myAddress && $gameContractInstance.playerEtraxID_($myAddress, (err, map) => {
            if (err) {
                reject(err);
            } else {
                let myRound = map[4].toString();
                $gameContractInstance.roundDividend(myRound, (err, dividend) => {
                    if (err) {
                        reject(err);
                    } else {
                        if (dividend.toString() == "0") {
                            $gameContractInstance.keyDividend((err, thisDividend) => {
