...
contract XMan {
    ...
    function createPlayer(address _addr, address _referrer) private {
        ...
        if (pIDxAddr_[_addr] == 0) {
            ...
            playerxID_[nPlayerNum] = DataSets.PlayerInfo(_addr, emptyAddr, emptyArr2, emptyArr, 0, 0, 0, 0, 0);
    ...
    function coinRollIn(address _referrer) isWithinLimits(msg.value) public payable {
        ...
        if (playerxID_[pIDxAddr_[_addr]].turnBonus >= 3) {
    ...
    function bonusToReferrer(uint allCoin, address _addr) private {
        ...
        if (referees != emptyAddr) {
            ...
            if (reagentId != 0)
            {
                addReferreraddCoin(playerxID_[reagentId].addr, allCoin * 5 / 100);
            }
            else
            {
    ...
    */
    function gameOver() public {
        ...
        if (totalPot > 0) {
            uint count = 0;
            if (Luckyer[3] != emptyAddr) {
                transferCoin(Luckyer[3], totalPot * 4 / 10);
                playerEtraxAddr_[Luckyer[3]].allEarning += totalPot * 4 / 10;
            }
            for (uint i = 0; i < 3; i++) {
            ...
            if (count > 0) {
                for (uint j = 0; j < 3; j++) {
                    if (Luckyer[j] != emptyAddr) {
                        transferCoin(Luckyer[j], totalPot * 6 / 10 / count);
                        playerEtraxAddr_[Luckyer[j]].allEarning += totalPot * 6 / 10 / count;
                    }
                }
            }
        }
    ...
    function withDraw() public {
        address _addr = msg.sender;
        transferCoin(_addr, playerxID_[pIDxAddr_[_addr]].turnBonus);
