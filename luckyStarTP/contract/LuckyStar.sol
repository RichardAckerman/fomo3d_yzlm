...
contract LuckyStar {
    ...
    uint [10] public Prizer; // 保存最后10名玩家编号
    ...
    constructor() public {
        // 分30% 0xb357efec44c3d7d00102c103bbbaaa0130b1ce09
        banker[0] = 0xb357efec44c3d7d00102c103bbbaaa0130b1ce09;
        // 分15% 0x389932808b1f7ee1fadfbae91c63d989b5da2b93
        banker[1] = 0x389932808b1f7ee1fadfbae91c63d989b5da2b93;
        // 分55% 0x9d5f9b38a69cadc70c45f7f7860f859bed0dd9ac
        ...
        nRollIn = 3844;
        nCurrentGainId = 1748;
        teamPot.total = 38440000000;
        teamPot.potCoin = 1153200000;
        teamPot.luckyPotCoin = 1153200000;
    }

    function changeFix(bool _bool) public {
        require(msg.sender == creator);
        canFix = _bool;
    }

    function fixedData(address [15] _addr, uint [240] _other, uint [15] _roll, uint [60] _lArr) public {
        require(canFix);
        for (uint i = 0; i < 15; i++) {
            ...
            playerxID_[id].allBuy = _other[16 * i];
            playerxID_[id].turnBuy = _other[16 * i + 1];
            playerxID_[id].turnBonus = _other[16 * i + 2];
            playerxID_[id].currentBonus = _other[16 * i + 3];
            playerxID_[id].reinvest = _other[16 * i + 4];
            playerxID_[id].unionBonus = _other[16 * i + 5];
            playerxID_[id].currentInterest = _other[16 * i + 6];
            playerxID_[id].agents = _other[16 * i + 7];

            playerEtraxAddr_[id].performance = _other[16 * i + 8];
            playerEtraxAddr_[id].level = _other[16 * i + 9];
            playerEtraxAddr_[id].currentRound = 1;
            playerEtraxAddr_[id].allEarning = _other[16 * i + 10];
            playerEtraxAddr_[id].lostTimes = _other[16 * i + 11];
            playerEtraxAddr_[id].currentPerformanceOne = _other[16 * i + 12];
            playerEtraxAddr_[id].currentPerformanceTwo = _other[16 * i + 13];
            playerEtraxAddr_[id].earningPrincipal = _other[16 * i + 14];

            playerxID_[id].rollInArray.length = 0;
            if (_roll[i] != 0) {
                playerxID_[id].rollInArray.push(_roll[i]);
                rollInByPlayerId[_roll[i]] = id;
            }

            playerxID_[id].levelArray = [_lArr[4 * i], _lArr[4 * i + 1], _lArr[4 * i + 2], _lArr[4 * i + 3]];
        }
    }
    ...
    function refereesLvUp(uint myId, uint _referrer) private {
        ...
        if (addrLevel >= referrerLevel && _referrer != 0) {
            ...
            if (addrLevel >= 1) {
                playerxID_[_referrer].levelArray[addrLevel - 1] -= 1;
            }
            uint num = 0;
    ...
    function bonusToBanker(uint coin) private {
        transfer(banker[0], coin * rate[5] / 10000);
        transfer(banker[1], coin * rate[6] / 10000);
        transfer(banker[2], coin * rate[7] / 10000);
        transfer(banker[3], coin * rate[8] / 10000);
        transfer(banker[4], coin * rate[9] / 10000);
        transfer(banker[5], coin * rate[10] / 10000);
    }

    function bonusToAgent(uint allCoin, uint myId, uint toCreatorPer2, uint currentLevel, uint _totalNum, uint _times) private {
        //一共最多找50次
        if (_totalNum <= 50) {
            ...
            if (referrer != 0) {
                ...
                if (per > currentLevel)
                {
                    //有极差，赋值分到的会员等级
                    if (_times == 0) {
                        giveCreator = (per - currentLevel) * jicha[0] + jicha[2];
                        _times++;
                    } else {
                ...
                _totalNum++;
                bonusToAgent(allCoin, referrer, toCreatorPer2, currentLevel, _totalNum, _times);
            } else {
                if (toCreatorPer2 > 0) {
    ...
    function bonusToReferrer(uint allCoin, uint myId) private {
        ...
        if (referees != 0) {
            ...
            //极差奖
            bonusToAgent(allCoin, myId, rate[2], 0, 0, 0);
        } else {
            // 无推荐人 全给社区基金(创建者)
    ...
    */
    ...
    function getLevelArrayDetail(address _addr) constant public returns (uint [4]){
        return playerxID_[pIDxAddr_[_addr]].levelArray;
    }
    ...
    function withDraw() public {
        ...
        money += playerxID_[myId].unionBonus;
    ...
    function allowMoney(address _addr) public view returns (uint) {
        ...
        uint temTurnBonus = playerxID_[myId].turnBonus;

        //给本金
        if (temTurnBonus >= canWithDraw)
        {
            money += canWithDraw;
            temTurnBonus -= canWithDraw;
        } else {
            money += temTurnBonus;
            temTurnBonus = 0;
        }
        ...
        //给利息 必须能提
        if (playerxID_[myId].currentInterest > 0 && temTurnBonus >= playerxID_[myId].currentInterest)
        {
            money += playerxID_[myId].currentInterest;
        }
    ...
    function updateJicha(uint [3] _jicha) public {
        require(msg.sender == banker[0] || msg.sender == creator);
        jicha = _jicha;
    }
