class FinalBonusUnit extends eui.Scroller {

    public constructor() {
        super();
    }

    private grandPrize = "0";
    private smallPrize = "0";
    private sourceArr = [];
    ...
    private getData() {
        ...
        $gameContractInstance.grandPrize((err3, coin) => {
            if (err3) {
                console.log("grandPrize", err3);
            } else {
                this.grandPrize = $toFixedDecimal(coin);
            }
        });
        $gameContractInstance.smallPrize((err3, coin) => {
            if (err3) {
                console.log("smallPrize", err3);
            } else {
                this.smallPrize = $toFixedDecimal(coin);
                this.getQueueId(0);
            }
        });
    }
    ...
    private getQueueId(i) {
        // 查幸运玩家的id列表
        $gameContractInstance.Luckyer(i, (err1, id) => {
            if (err1) {
                    ...
                    if (err2) {
                        ...
                        if (id.toString() != "0") {
                            let obj = {
                                index: i + 1,
                                address: addr,
                                extractCoin1: i == 9 ? this.grandPrize : this.smallPrize
                            };
                            this.sourceArr.push(obj);
                            this.getList();
                        }
                        if (i < 9 && id.toString() != "0") {
                            i++;
                            this.getQueueId(i);
                        }
                    }
