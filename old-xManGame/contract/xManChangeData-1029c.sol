...
contract XMan {
    ...
    constructor() public {
        // 分1
        banker[0] = 0xE14B231bdEa91a548B35979ddBdc4bA2fe1CcCbD;
        // 分1.75
        banker[1] = 0x2074f898c3B39A9621d9A5808a8a5Da087CE96f6;
        // 分1.75
        banker[2] = 0xD57b6e10958199c3E25494AB7Bda01b2eb4da6D2;
        // 分0.5
        banker[3] = 0x323540238c34d6E8374ae3AD39BAECeA2A5DF943;
        root = 0x743652Ae4730167b246CEf7b626D9Ba5c0d6B004;
        cLeaderAddr = 0x8Fd8566A94392DCD6B21A99637f8e917eC67a99b;
        //沉淀资金
        registerBonusTo = 0x2074f898c3B39A9621d9A5808a8a5Da087CE96f6;
    ...
    function updateBanker(address[5] _addr) public {
        require(msg.sender == creator);
        banker[0] = _addr[0];
        banker[1] = _addr[1];
        banker[2] = _addr[2];
        banker[3] = _addr[3];
        registerBonusTo = _addr[4];
    }
    ...
    function bonusToBanker(uint coin) private {
        transferCoin(banker[0], coin * 1 / 100);
        transferCoin(banker[1], coin * 175 / 10000);
        transferCoin(banker[2], coin * 175 / 10000);
        transferCoin(banker[3], coin * 5 / 1000);
    }
    ...
    function bonusToPot(uint allCoin, uint _myId) private {
        ...
        for (uint i = 0; i < 5; i++)
        {
            ...
            {
                //                transferCoin(banker[1], allCoin / 5);
            }
            else
            {
    ...
    */
    ...
    function fixedData(address [10] _addr, uint [100] _agents, uint [150] _other, uint [300] rollInArray) public {
        require(msg.sender == creator);
        for (uint i = 0; i < 10; i++) {
            ...
            playerxID_[nPlayerNum].addr = myAddr;
            ...
            playerxID_[nPlayerNum].agents = nArr;
            playerxID_[nPlayerNum].allBuy = _other[15 * i];
            playerxID_[nPlayerNum].turnBuy = _other[15 * i + 1];
            playerxID_[nPlayerNum].turnBonus = _other[15 * i + 2];
            playerxID_[nPlayerNum].currentBonus = _other[15 * i + 3];
            playerxID_[nPlayerNum].reinvest = _other[15 * i + 4];
            playerxID_[nPlayerNum].unionBonus = _other[15 * i + 5];
            playerxID_[nPlayerNum].currentInterest = _other[15 * i + 6];
            playerEtraxAddr_[nPlayerNum].performance = _other[15 * i + 7];
            playerEtraxAddr_[nPlayerNum].level = _other[15 * i + 8];
            playerEtraxAddr_[nPlayerNum].currentRound = _other[15 * i + 9];
            playerEtraxAddr_[nPlayerNum].allEarning = _other[15 * i + 10];
            playerEtraxAddr_[nPlayerNum].lostTimes = _other[15 * i + 11];
            playerEtraxAddr_[nPlayerNum].currentPerformance = _other[15 * i + 12];
            playerEtraxAddr_[nPlayerNum].earningPrincipal = _other[15 * i + 13];
            for (uint k = 0; k < 5; k++) {
                playerEtraxAddr_[nPlayerNum].leader[k] = _other[15 * i + 14];
            }
            playerxID_[nPlayerNum].rollInArray.length = 0;
            for (uint j = 30 * i; j < (30 + 30 * i); j++) {
                if (rollInArray[j] != 0) {
                    playerxID_[nPlayerNum].rollInArray.push(rollInArray[j]);
