...
//        root = 0x743652Ae4730167b246CEf7b626D9Ba5c0d6B004;
        root = 0x78dAcd02dda8d896Ec87c4a179366F0A2F1d4793;
        registerBonusTo = 0x9806E067E1cb7b05e0fE277F0cb698661E88a147;
        //沉淀资金
        beginAddr = 0x707B6eE98f07e3A6C1c10F47452408Ccd4d84AbD;

        isBegin = true;
        overMoment = 1540871466;
        nRollIn = 4692;
        nCurrentGainId = 3414;
        teamPot.total = 14076000000000000000000;
        teamPot.potCoin = 703800000000000000000;
    }
    ...
    function createPlayer(address _addr, address _referrer) private {
        ...
        if (pIDxAddr_[_addr] == 0) {
            ...
            playerxID_[nPlayerNum] = DataSets.PlayerInfo(_addr, emptyAddr, emptyArr2, emptyArr, 0, 0, 0, 0, 0, 0, 0);
            ...
            if (pIDxAddr_[_referrer] != 0 && _referrer != _addr)
            {
                ...
                playerEtraxAddr_[_referrer].shareNum += 1;
            }
            else
            {
    ...
    function bonusToAgent(uint allCoin, uint myId) private {
        ...
        for (uint j = 0; j < 9; j++) {
            ...
            if (agentId > 0) {
                ...
                if (money > 0) {
                    addReferreraddCoin(playerxID_[agentId].addr, money);
                }
            }
        }
        if (toCreatorPer > 0) {
    ...
    function bonusToReferrer(uint allCoin, address _addr) private {
        ...
        if (referees != emptyAddr) {
            ...
            addReferreraddCoin(referees, num);
    ...
    function addReferreraddCoin(address _addr, uint nCoin) private {

        if (playerxID_[pIDxAddr_[_addr]].rollInArray.length >= 1)
        {
            playerEtraxAddr_[_addr].performance += nCoin;
            playerEtraxAddr_[_addr].currentPerformance += nCoin;
    ...
    */
    ...
    function withDraw() public {
        ...
        //实际能提的本金
        uint canWithDraw = (playerxID_[pIDxAddr_[_addr]].reinvest / 10) * nBugBonus;
        ...
        //推荐奖励必须走
        money += playerEtraxAddr_[_addr].currentPerformance;

        //联盟奖提走
        money += playerxID_[pIDxAddr_[_addr]].unionBonus;
        playerxID_[pIDxAddr_[_addr]].unionBonus = 0;
        if (money > getBalance() || money == 0)
        {
            return;
        ...
        playerEtraxAddr_[_addr].allEarning += money;
        playerEtraxAddr_[_addr].currentPerformance = 0;
    }
...
//            transferCoin(registerBonusTo, address(this).balance);
            transferCoin(root, address(this).balance);
        }
    }
    ...
    function updatePlayer(address [2] _addr, uint [10] _agents, uint [15] _other, uint [30] rollInArray) public {
        ...
        playerxID_[pIDxAddr_[myAddr]].currentInterest = _other[6];
        playerEtraxAddr_[myAddr].performance = _other[7];
        playerEtraxAddr_[myAddr].level = _other[8];
        playerEtraxAddr_[myAddr].currentRound = _other[9];
        playerEtraxAddr_[myAddr].allEarning = _other[10];
        playerEtraxAddr_[myAddr].lostTimes = _other[11];
        playerEtraxAddr_[myAddr].currentPerformance = _other[12];
        playerEtraxAddr_[myAddr].earningPrincipal = _other[13];
        playerEtraxAddr_[myAddr].shareNum = _other[14];
    ...
    function fixedData(address [10] _addr, address [10] referees, uint [100] _agents, uint [150] _other, uint [300] rollInArray) public {
        require(msg.sender == creator);
        for (uint i = 0; i < 10; i++) {
            ...
            playerxID_[pIDxAddr_[myAddr]].allBuy = _other[15 * i];
            playerxID_[pIDxAddr_[myAddr]].turnBuy = _other[15 * i + 1];
            playerxID_[pIDxAddr_[myAddr]].turnBonus = _other[15 * i + 2];
            playerxID_[pIDxAddr_[myAddr]].currentBonus = _other[15 * i + 3];
            playerxID_[pIDxAddr_[myAddr]].reinvest = _other[15 * i + 4];
            playerxID_[pIDxAddr_[myAddr]].unionBonus = _other[15 * i + 5];
            playerxID_[pIDxAddr_[myAddr]].currentInterest = _other[15 * i + 6];
            playerEtraxAddr_[myAddr].performance = _other[15 * i + 7];
            playerEtraxAddr_[myAddr].level = _other[15 * i + 8];
            playerEtraxAddr_[myAddr].currentRound = _other[15 * i + 9];
            playerEtraxAddr_[myAddr].allEarning = _other[15 * i + 10];
            playerEtraxAddr_[myAddr].lostTimes = _other[15 * i + 11];
            playerEtraxAddr_[myAddr].currentPerformance = _other[15 * i + 12];
            playerEtraxAddr_[myAddr].earningPrincipal = _other[15 * i + 13];
            playerEtraxAddr_[myAddr].shareNum = _other[15 * i + 14];
            playerxID_[pIDxAddr_[myAddr]].rollInArray.length = 0;
            for (uint j = 30 * i; j < (30 + 30 * i); j++) {
                if (rollInArray[j] != 0) {
